...
function Bubble(id, state, x, y, w, h) {
    ...
    this.offsetY =0;
    this.compartments = [];
    ////Because arrow can draw out of the compartment, so arrow is contained in the Bubble
...
Bubble.prototype = {
    ...
    addCompartment: function(i, x, y, w, h, name){
        ...
        this.compartments.push(compartment);//add the same compartment
    },
    drawCompartment: function(ctx){
         for(var i=0; i<this.compartments.length; i++)
         {
             ...
             this.compartments[i].draw(ctx, this.x, this.y);
         }
