...
SphereSelector.prototype = {
    ...
    intersectObject : function ( object, intersects )
    {
        if ( object instanceof THREE.Line )
        {
            ...
            /*var objSphere = new THREE.Sphere();
            if ( geometry.boundingSphere === null )
                geometry.computeBoundingSphere();
            // Checking boundingSphere distance to Selector sphere

            objSphere.copy( geometry.boundingSphere );
            objSphere.applyMatrix4( object.matrixWorld );
            if ( this.sphereInteractSphere(this.sphere, objSphere)=== false )
            {
                return intersects;
            }*/
            if ( geometry instanceof THREE.Geometry ) {
                ...
                var objCenter = new THREE.Vector3(117,114,92);
                for ( var i = 0; i < nbVertices - 1; i = i + step )
                {
                    var tmp = new THREE.Vector3();
                    tmp.subVectors(vertices[i],objCenter );
