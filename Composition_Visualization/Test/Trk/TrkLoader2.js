...
TrkLoader.prototype = {
    ...
    load: function (url, callback) {
        ...
        function onloaded(event) {
            if (event.target.status === 200 || event.target.status === 0) {
                var object = scope.parse(event.target.response || event.target.responseText);
    ...
    parse: function (data) {
        ...
        return this.buildGeometry(data, header);
    },
    buildGeometry: function(data, header)
    {
        ...
        var number=[];
        if(n_s || n_p)  //we must let n_s and n_p ==0
        {
            alert("Unsupport Format!");
        }
        for(var i=0; i<n; ++i)  //size = 65535; //WebGL limits type of index buffer values to 16-bit.
        {
            var m = this.parseInt32Array(data, current_offset, 1);
            current_offset += 4; //1 int
            number.push(m);
            current_offset += 4 * 3 * m;// m * 3 float vertex data
        }
    ...
    parseTrack: function (data, offset, object, n_s, n_p) {
        ...
        geometry.addAttribute( 'position', new Float32Array( m * 3 ), 3 );    //name, type, numItems, itemSize
