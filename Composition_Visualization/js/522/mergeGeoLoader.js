...
GeometryLoader.prototype = {
    ...
    parse: function ( text ) {
        ...
        var mergeGro = new THREE.Geometry();
        ...
        for(var i=0;i<totalFiberNum;i++)
        {
            ...
            var vertexPosition = [];
            var vertexColor = [];
            for(var j = 1;j<=totalVertexNum;j+=3)
            {
            ...
            geometry = new RibbonGeometry(vertexPosition, 1, vertexColor);
            geometry.uuid = i;
            THREE.GeometryUtils.merge(mergeGro, geometry);
