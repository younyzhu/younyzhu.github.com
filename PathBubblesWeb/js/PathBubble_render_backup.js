...
PATHBUBBLES.Render = function(canvas, scene){
    ...
    canvas.addEventListener('mousemove', function (e) {
        ...
        if (_this.dragging) {
            ...
            {
                _this.selection[0].offsetX += offsetX;
                _this.selection[0].offsetY += offsetY;
            }  //mouse move relative to the navigation viewpoint
            if(_this.selection[0].GROUP == true)
            {
                for(var i=0; i<_this.selection[0].groupObj.children.length; ++i)
                {
                    _this.selection[0].groupObj.children[i].offsetX += offsetX;
                    _this.selection[0].groupObj.children[i].offsetY += offsetY;
                }
                _this.selection[0].groupObj.offsetX += offsetX;
                _this.selection[0].groupObj.offsetY += offsetY;
            }
            ...
            switch (_this.expectResize) {
                case 0:
                    _this.selection[0].shape.x = mx;
                    _this.selection[0].shape.y = my;
